...
mod tests {
    ...
    #[test]
    fn strings_are_counted_and_not_terminated() {
        let body = W::new().s("motd").into_body();
        assert_eq!(body, vec![4, 0, b'm', b'o', b't', b'd']);
    }
    ...
    fn a_message_round_trips() {
        ...
        assert_eq!(msg.ty, T::Write as u8);
        assert_eq!(msg.tag, 7);
        let mut r = R::new(msg.body);
        assert_eq!(r.u32(), Some(4));
        assert_eq!(r.u64(), Some(9));
        assert_eq!(r.s(), Some("hello"));
    }

    #[test]
    fn a_truncated_message_is_not_a_message() {
        let m = W::new().u32(4).frame(T::Read as u8, 1);
        assert!(unframe(&m[..m.len() - 1]).is_none());
    }

    #[test]
    fn a_lying_length_does_not_panic_the_kernel() {
        // a server we did not write claims a 200-byte string in a 10-byte body
        let mut b = vec![200u8, 0];
