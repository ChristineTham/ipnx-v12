...
if (!crossOriginIsolated) {
  ...
  diag.textContent = `state: secure=${isSecureContext} proto=${location.protocol} sw-api=${hasSW} page=${ctl} build=1788128781`;
  d.appendChild(diag);
  if (hasSW)
...
const resp = await fetch("../build/rootfs.json?v=1788128781");
...
const kernelWasm = await (await fetch("../build/kernel.wasm?v=1788128781")).arrayBuffer();
setStatus("booting…");
...
(async () => {                                    // the toolchains stream in
  ...
  try {
    const man = await (await fetch("../build/overlays.json?v=1788128781")).json();
    ...
    for (const prof of profs) {
      ...
      for (let i = 0; i < parts.length; i++) {     // b64 text across parts
        ...
        const ov = await (await fetch(`../build/${parts[i]}?v=1788128781`)).json();
...
statusEl.title = "build 1788128781";
document.getElementById("brand").title = "build 1788128781";
const bs = document.createElement("button");
bs.style.cssText = "all:unset;cursor:pointer;color:#4a5866;font-size:10.5px;margin-left:8px;user-select:text";
bs.textContent = "b1788128781";
bs.title = "the build stamp — click to copy";
bs.addEventListener("click", () => {
  navigator.clipboard?.writeText("b1788128781").then(() => toast("build stamp copied: b1788128781"));
});
statusEl.after(bs);
consT.term.focus();
