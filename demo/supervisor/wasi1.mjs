...
const ENVIRON = ["PYTHONHOME=/", "PYTHONDONTWRITEBYTECODE=1", "HOME=/usr/kitty", "TERM=dumb"];
...
export function makeWasi(ctx) {
  ...
  // WASI has no cwd; the kernel does. A path opened against the preopen
  // arrives relative (WASI strips leading slashes), so the shim cannot tell
  // a was-absolute "/lib/x" (Python de-slashes its stdlib) from a genuinely
  // cwd-relative "hello.py". Resolution: the kernel's cwd version wins when it
  // EXISTS, else fall back to root — so `python hello.py` from /usr/kitty finds
  // home while `lib/python3.14` still falls through to /lib. cwd is fixed at
  // exec (no chdir in WASI) so it is read once.
  let _cwd;
  function cwd() {
    if (_cwd !== undefined) return _cwd;
    const kfd = sysTx(T.OPEN, ["."], 0, OREAD, 0, 0, 0);
    if (kfd < 0) return (_cwd = "/");
    const n = sysTx(T.FD2PATH, [], kfd, 0, TXSIZE, 0, 0);
    sys(T.CLOSE, kfd, 0, 0, 0, 0);
    _cwd = n > 0 ? new TextDecoder().decode(txView().slice(0, n)) : "/";
    return _cwd;
  }
  function norm(p) {
    const out = [];
  ...
  function exists(path) {
    return sysTx(T.STAT, [path], 0, NOCOPY, 4096, 0, 0) >= 0;
  }
  function resolve(dirfd, ptr, len) {
    ...
    if (c !== "/") {
      const cp = norm(c + "/" + raw);
      if (exists(cp)) return cp;                     // cwd wins when it's real
    }
    return norm("/" + raw);                          // else root (stdlib, absolutes)
  }
  ...
  const imports = {
    ...
    // environ: PYTHONHOME=/ pins CPython's stdlib to /lib/python3.14 regardless
    // of cwd (getpath would otherwise probe relative to the working directory
    // now that the shim honours it). Harmless to the other citizens.
    environ_sizes_get: (countP, sizeP) => {
      const e = ENVIRON;
      dv().setUint32(countP, e.length, true);
      dv().setUint32(sizeP, e.reduce((t, s) => t + s.length + 1, 0), true);
      return E.success;
    },
    environ_get: (envP, bufP) => {
      const enc = new TextEncoder();
      let o = bufP;
      ENVIRON.forEach((v, i) => {
        dv().setUint32(envP + 4 * i, o, true);
        const b = enc.encode(v); m8().set(b, o); o += b.length; m8()[o++] = 0;
      });
      return E.success;
    },
