...
libbio, libregexp and libString as `libp9.a`, and twenty-five real commands including
`ls`, `sed`, `grep` (its yacc grammar regenerated by the host's bison — the same door
rc's `syn.y` uses next) and `sort`. V10 growth waits, per direction, for the parent
