...
| **not built** | 6 programs (`build/failed` is the list, with each reason). Three cannot be on this machine as they are written: `syscall` calls every call through one pointer type (`syscall.c:32`), which wasm's typed indirect calls refuse; `aux/vmware`'s two read 386's registers by name for VMware's I/O port (`backdoor.c`); `7l` allocates from the loader's `end` with `brk` (`7l/compat.c:77`), where this machine puts the argument block. `7a` does not build on Plan 9 either: `IOUNIT` is defined in no header (`7a/a.h:23`). Still to fix: `grap`'s lexer and `gs`. `units` needs `/lib/units`, which is not vendored |
