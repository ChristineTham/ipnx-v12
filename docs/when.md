...
| **built** | **all 36 libraries** — libdynld with Plan 9's "unimplemented" machine file, as ten of its architectures have — libsec among them, its curve tables made by the system's own `mpc`, and libthread, its threads coroutines by the machine's `setjmp`/`longjmp`; **APE's 12** (`ap`, `9`, `bsd`, `draw`, `fmt`, `l`, `mp`, `net`, `regexp`, `sec`, `utf`, `v`), `libap` with a wasm machine directory (`ape/lib/ap/wasm`: `_start`, the call stubs, `setjmp`, `brk`, the atomics); and **459 programs** in `/pkg/system/2026.09.24/wasm/bin`, rc and init among them as Plan 9's own, the compilers, loaders and assemblers, awk, APE's `sh`, `sed`, `diff`, `patch` in `bin/ape`; and **Plan 9's rc scripts** (`rc/bin`, vendored whole) in the package's `rc/bin`, bound after the programs as `/lib/namespace:27` binds them, less the startup files `/profile` replaces. Grammars are Plan 9's yacc's and lexers Plan 9's lex's, and a source a recipe makes is made by running the recipe on the system, in the second pass |
| **not built** | 63 programs (`build/failed` is the list, with each reason): directory libraries not yet made (`bzip2`, `fossil`, `vac`, `usb`), rc's `replica/*` wrappers, the mkfile's quoted `-D` flags (`faces`, `upas`), kencc features the derivation does not yet give clang (`ar`, `dc`, `cb`, `db`, `ftpfs`), `lp`'s APE programs, `cifs`'s `long double`, `vmware`'s 386 registers. `7a` does not build on Plan 9 either: `IOUNIT` is defined in no header (`7a/a.h:23`). `syscall` calls every call through one pointer type (`syscall.c:32`), which wasm's typed indirect calls refuse. `units` needs `/lib/units`, which is not vendored |
