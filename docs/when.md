...
| **built** | **all 36 libraries** — libdynld with Plan 9's "unimplemented" machine file, as ten of its architectures have — libsec among them, its curve tables made by the system's own `mpc`, and libthread, its threads coroutines by the machine's `setjmp`/`longjmp` — and **401 programs** in `/pkg/system/2026.09.24/wasm/bin`, rc and init among them as Plan 9's own |
| **not built** | 120 programs — those needing APE (`awk`, `troff`, `eqn`, `pic`, `grap`, `spin`, `compress`, `lp`), the compilers and assemblers, and kencc features the derivation does not yet give clang — `fauth` 25, libsec's `sha1` and ciphers, `yylex` (lex's library), the compilers' generated files. `build/failed` is the list. libthread's `proccreate` and `threadexec` run, over `RFMEM` |
