...
| **built** | **all 36 libraries** — libdynld with Plan 9's "unimplemented" machine file, as ten of its architectures have — libsec among them, its curve tables made by the system's own `mpc`, and libthread, its threads coroutines by the machine's `setjmp`/`longjmp`; **APE's 12** (`ap`, `9`, `bsd`, `draw`, `fmt`, `l`, `mp`, `net`, `regexp`, `sec`, `utf`, `v`), `libap` with a wasm machine directory (`ape/lib/ap/wasm`: `_start`, the call stubs, `setjmp`, `brk`, the atomics); and **511 programs** in `/pkg/system/2026.09.24/wasm/bin`, rc and init among them as Plan 9's own, the compilers, loaders and assemblers, awk, APE's `sh`, `sed`, `diff`, `patch` in `bin/ape`; and **Plan 9's rc scripts** (`rc/bin`, vendored whole) in the package's `rc/bin`, bound after the programs as `/lib/namespace:27` binds them, less the startup files `/profile` replaces. Grammars are Plan 9's yacc's and lexers Plan 9's lex's, and a source a recipe makes is made by running the recipe on the system, in the second pass |
| **not built** | 6 programs (`build/failed` is the list, with each reason). Three cannot be on this machine as they are written: `syscall` calls every call through one pointer type (`syscall.c:32`), which wasm's typed indirect calls refuse; `aux/vmware`'s two read 386's registers by name for VMware's I/O port (`backdoor.c`); `7l` allocates from the loader's `end` with `brk` (`7l/compat.c:77`), where this machine puts the argument block. `7a` does not build on Plan 9 either: `IOUNIT` is defined in no header (`7a/a.h:23`). Still to fix: `grap` (`struct symtab` incomplete in its lexer's file) and `gs` (`arch.h` names no case for this machine, and its `wasm.h` is `genarch`'s output, built with `pcc` and run on the target, `gs/mkfile:148`). `units` needs `/lib/units`, which is not vendored |
