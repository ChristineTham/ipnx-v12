...
|---|---|---|---|
| **Node** | `poc/supervisor` (frozen) | V8, Workers + SAB | **green, 133** (floor 131) — the oracle; also the dev loop |
| **Browser tab** | `poc/browser` (frozen) → `hosts/browser` (M5) | the page's own JIT, Workers + SAB | **green, 133** (floor 131) in Chrome 148 on the reference; **live at [christham.net/ipnx-v12](https://christham.net/ipnx-v12/)**; Rust core in wasm is M5 |
| **macOS** | `hosts/macos` | wasmtime 37, Cranelift JIT | **green, 133** (floor 131), headless; the app shell is M3 |
