...
| | |
|---|---|
| **without emca** | they connect straight to the host's screen, keyboard and mouse. IPNX is a CLI with no windows — a mode that must keep working. **The host receives the interrupt key** — `^C` on a terminal — and the console posts *"interrupt"* to its note group, as `rio` does for a window (Christine, 2026-09-23). Like `rio`, it does not echo the key: the terminal's echo of control characters is off while the host runs |
