...
## P6 — the scheduler *(added 2026-09-21; the switch is in, 2026-09-22)*
...
| **next** | **the clock**: the machine's half of `timerintr`, entering `hzclock` at HZ — `m->ticks++`, `accounttime()`, `checkalarms()`, `hzsched()`. It comes BEFORE preemption because preemption is one of the four things it does (RESEARCH §15.1); `updatecpu`, `reprioritize`, `cputime` and `alarm` are waiting on it too. **Then preemption**, then notes. The switch they need is — one per process, named for `setlabel`/`gotolabel` and not for any engine's word for it; then `port/proc.c`'s `Rendez`, `sleep`, `wakeup`, `tsleep`, `ready`, `runproc`, `yield`, `hzsched` and the twelve process states (`portdat.h:610`); then preemption; then notes — `postnote`, this machine's answer to `notify(Ureg*)`, `#p/<n>/note`, `RFNOTEG`, `alarm`; then `rendezvous`. About **425 lines of `port/proc.c`**, and not a line of invention |
