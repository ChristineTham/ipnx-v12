...
| **done** | **`rendezvous` and the process controls** (2026-09-23): `Rgrp` and `RFREND`, `start`/`stop`/`waitstop`/`hang`/`nohang` with `Stopped` and `procstopwait`, `Broken` and `NBROKEN`, and `psstate`. RESEARCH §15.4. **196 kernel tests**. P6 is built but for tracing (`startstop`, `startsyscall`, `/proc/n/syscall`, `profile`) |
