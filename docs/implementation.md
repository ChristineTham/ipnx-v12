...
| **next** | **the clock**: the machine's half of `timerintr`, entering `hzclock` at HZ — `m->ticks++`, `accounttime()`, `checkalarms()`, `hzsched()`. It comes BEFORE preemption because preemption is one of the four things it does (RESEARCH §15.1); `updatecpu`, `reprioritize`, `cputime` and `alarm` are waiting on it too |
| **then** | preemption; then notes — `postnote`, this machine's answer to `notify(Ureg*)`, `#p/<n>/note`, `RFNOTEG`, `alarm`; then `rendezvous`. About **425 lines of `port/proc.c`** in all, and not a line of invention |
