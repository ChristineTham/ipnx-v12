...
|---|---|---|---|
| **1 `#/`** | Plan 9's root device: a fixed small read-only table, `rootwrite` is `error(Egreg)`, holding empty mount points (`devroot.c`, 261 lines — Plan 9's table also holds `/boot`, the files compiled into the kernel image. **This system needs no equivalent**: the host reads `/namespace` and builds `/` from it, so nothing must be carried inside the kernel). **The plan's one addition to the kernel**, replacing `#M`+`#V` (243 lines) — net, the kernel shrinks | — | P1 step 2 |
| **2 the root file server** ⚠ *the DELETE half also depends on **step 5** — measured 2026-09-04: the rootfs is 49MB and a guest caps at 16MB, so no userspace server can hold it until Go and Python become packages ([RESEARCH §9.24](../RESEARCH.md))* | a **userspace** 9P server for the rootfs — `plan9/sys/src/cmd/ramfs.c` (**945 lines**; plan9-stock's is 907 and the two differ — 9legacy is what a claim is checked against) is the reference shape. It serves the seed the host provides through `#Z`, so the host stays a storage box and the tree is a process | `#M`, `#V`, `ram_*`, `snap_*` from the kernel | 1 |
| **3 the boot path — `/namespace`, read by the HOST** ⚠ *namespace half landed 2026-09-17; the **rc half is still the gap** — its name and location, reserved in [design.md](design.md), is what blocks step 5's flip* | **`/` is a PROJECT instantiated from `/template/system`**, and **boot reads `/namespace` — the instance's own configuration, in `/` because `/` is the project. The template is NOT consulted at boot** ([design.md](design.md) 2026-09-04). `/lib/namespace` becomes **`/namespace`** and loses its `bind #w /dev/window` line. **The HOST reads it and creates the rest of `/` from it** — no kernel addition and no `#/boot` equivalent, because the host already owns the storage. Still rc, which is founding (*"no systemd — boot is rc plus a namespace file"*), and `init`/`rc` is the command that runs in the result. The **skeleton** is the small editable tree copied in at instantiation; the bulk (Go, Python, the stdlib) **binds** from the store as packages — *bind what stays shared, copy what becomes yours* — which is what makes step 2's server fit ([RESEARCH §9.24](../RESEARCH.md)). **Instantiate once, boot many times**: an instantiated `/` may be modified and saved back as a new template | `init.c`'s implicit-root assumption; `-w` and every window path in `init.c` | 2 |
...
| **5 Go and Python, as packages** ⚠ *this cell described pkg **v1**, not the spec it cites — corrected 2026-09-04, [RESEARCH §9.25](../RESEARCH.md)* | the `pkg` design is spec'd ([type.md](type.md)): **`/pkg/<name>` is a DECLARATION FILE** — a list of bindings plus commands — and the bytes live in **`/store/<name>/<version>`**, fetched once, verified, immutable, and **BOUND, never copied**; `pkg remove` is an unbind. `cmd/pkg.c` implemented v1 (copy into `/pkg/<name>/<version>`) when this cell was written, and **only bind-from-store shrinks the running tree**, which is what step 2 needs. `/lib/pkg/registries`. Each ships as a package — the binary, its stdlib (Python's measured 21-file subset), its bindings, its install commands — from a **local registry** for the demo. **Landed 2026-09-04/15:** `storefs`, `pkg` v2, and the **`tree`** verb — content that is a directory is ONE pinned digest over a manifest ([design.md](design.md) decision 122, [RESEARCH §9.27](../RESEARCH.md)). **Host plumbing landed 2026-09-16** ([RESEARCH §9.28](../RESEARCH.md)): `--host <dir>` roots `#Z` at a durable directory, proved across two boots, with **no kernel change**. **The move is PROVED 2026-09-17** ([RESEARCH §9.29](../RESEARCH.md)): `mk.sh` materialises the store (python 541 files / 41 MB, go) with five-line declarations, and `rc/storeproof` runs REAL CPython and REAL Go **from the store** over 9P. **The flip — dropping them from the seed — waits on step 3's rc script having a name**, because `newns`'s verbs (`bind | mount | cd | clear`) cannot start `storefs` | — | `pkg`'s `#H` fetch (P1 step 5) |
...
**Acceptance:** `ipnx` boots through `#/` → *(step 3's rc script — its name is
a gap)* → the root server → `init` (which does `newns` from `/lib/namespace`,
as it already does) → `rc`; `pkg install go` and `pkg install python` from the local
