...
| **3 the boot path — ⚠ ONLY THE NAME IS A GAP** | **It IS rc**, and that is founding, not incidental: *"no systemd — **boot is rc plus a namespace file**"* (CLAUDE.md's three refusals). The **namespace-file half already landed** (M2, 2026-08-29): `/lib/namespace`, read by `init` through `newns()` as its own header says — so `newns` is already in the right place and stays there. This step is the **rc half**: a script that attaches the root server and binds it over `/`. `/lib/namespace` loses its `bind #w /dev/window` line. **It must not be called `/boot`** — *"we can't call something /boot and refer to something other than a bootloader"*, and *"not that we should implement a bootloader"* (2026-09-04; the 2026-09-02 naming rule, [design.md](design.md)). Plan 9's own equivalent is C (`boot.c`) and has no `bootrc`, but that is exactly the departure the systemd refusal makes deliberately. **GAP: where the script lives and what it is called.** `/rc/` holds scripts named for their program (`/rc/emca`), and this one has no program to be named for | `init.c`'s implicit-root assumption; `-w` and every window path in `init.c` | 2 |
