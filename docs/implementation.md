...
## P6 — the scheduler *(added 2026-09-21; built 2026-09-23, but for tracing)*
...
| **done** | **`rendezvous` and the process controls** (2026-09-23): `Rgrp` and `RFREND`, `start`/`stop`/`waitstop`/`hang`/`nohang` with `Stopped` and `procstopwait`, `Broken` and `NBROKEN`, and `psstate`. RESEARCH §15.4. **196 kernel tests**. P6 is built but for tracing (`startstop`, `startsyscall`, `/proc/n/syscall`, `profile`), and `^C`, which is not the kernel's |
...
| **depends on** | P5 |
| **acceptance** | `sleep(2)` is a `tsleep` on a `Rendez` and a second process runs during it ✓; a guest in a tight loop does not stop the system ✓; **`^C` interrupts a command**, so rc's `Trapinit` stops being a stub — `Trapinit` is `plan9.c`'s and a note interrupts a command ✓, but **Plan 9's kernel console turns no key into a note**: `rio` writes *"interrupt"* to the note group (RESEARCH §15.3). The key needs whatever plays `rio`'s part on the surface |
