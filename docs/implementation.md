...
## P6 — the scheduler *(added 2026-09-21; the switch and the clock are in, 2026-09-22)*
...
| **done** | **the clock, and preemption with it** (2026-09-22): `Mach`, `timersinit`, `timerintr` → `hzclock` → `accounttime`, `hzsched`, `rebalance`; the machine's `clockintr` on wasmtime's epoch; `sched()` at the interrupt's tail. **A guest in a tight loop does not stop the system.** Building it exposed two older faults, fixed with it: **pipes never blocked** (`qread` now sleeps on `q->rr`) and **exit never closed descriptors** (`closefgrp`). RESEARCH §15.2. **179 kernel tests** |
| **next** | **notes** — `postnote`, this machine's answer to `notify(Ureg*)`, `#p/<n>/note`, `RFNOTEG`, `checkalarms` and `alarm`, and `/proc/n/ctl`'s `kill` as the note Plan 9 posts rather than the direct `exits` it is now. `^C` is the acceptance test |
| **then** | `rendezvous`. About **425 lines of `port/proc.c`** in all, and not a line of invention |
