...
| **next** | **preemption**, then notes. The switch it needs is — one per process, named for `setlabel`/`gotolabel` and not for any engine's word for it; then `port/proc.c`'s `Rendez`, `sleep`, `wakeup`, `tsleep`, `ready`, `runproc`, `yield`, `hzsched` and the twelve process states (`portdat.h:610`); then preemption; then notes — `postnote`, this machine's answer to `notify(Ureg*)`, `#p/<n>/note`, `RFNOTEG`, `alarm`; then `rendezvous`. About **425 lines of `port/proc.c`**, and not a line of invention |
