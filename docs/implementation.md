...
|---|---|---|---|
| **1 `#/`** | Plan 9's root device: a fixed small read-only table, `rootwrite` is `error(Egreg)`, holding empty mount points (`devroot.c`, 261 lines — Plan 9's table also holds `/boot`, the files compiled into the kernel image. **The equivalent here is what step 3's remaining question needs**: something must carry `/template/system` before `/` exists). **The plan's one addition to the kernel**, replacing `#M`+`#V` (243 lines) — net, the kernel shrinks | — | P1 step 2 |
...
| **3 the boot path — `/template/system`** | **`/` is a PROJECT instantiated from the SYSTEM TEMPLATE**, and the boot path is that template's configuration ([design.md](design.md) 2026-09-04). No new vocabulary: `/profile`, `/pkg` and `/template` are already *one format, three registries* — a list of bindings plus commands — and booting **is** instantiating, with `init`/`rc` the command that runs in the result. **It IS rc**, which is founding: *"no systemd — **boot is rc plus a namespace file**"*. The **namespace half already landed** (M2, 2026-08-29): `/lib/namespace`, read by `init` through `newns()`, moves to **`/template/system/namespace`** and loses its `bind #w /dev/window` line. The **skeleton** is the small editable tree; the bulk (Go, Python, the stdlib) **binds** from the store as packages — *bind what stays shared, copy what becomes yours* — which is also what makes step 2's server fit ([RESEARCH §9.24](../RESEARCH.md)). **STILL OPEN — and it is not the name:** the bootstrap ordering, i.e. what reads `/template/system` before `/` exists to read it from. Plan 9 answers the same question with files compiled into the kernel image at `#/boot` | `init.c`'s implicit-root assumption; `-w` and every window path in `init.c` | 2 |
