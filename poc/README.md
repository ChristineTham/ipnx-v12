...
The test boot prints ninety `PASS` lines — forty-two from init (kernel, mount,
...
- **Real userspaces, both of them — and the Plan 9 one is now a system.**
  ...
  libbio, libregexp, libString — about 150 source files in `libp9.a` — and twenty-five
  real commands compiled unmodified: `cat echo ls wc cp mv rm mkdir chmod tee cmp tr
  uniq tail sort sed grep du date test touch sleep basename cleanname` and more, grep's
  yacc grammar regenerated by the host's bison at build time. The suite's pipelines run
  real `sed` rewrites, real `sort | uniq`, real anchored `grep`, real `date`, and rc
  conditionals driven by real `test`.
