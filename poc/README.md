...
set the COOP/COEP headers SharedArrayBuffer requires. The same suite passes in both
(measured in Chrome 148; the floor is 131, and with the demo's tour test the
count today is 132).

The test boot prints 132 `PASS` lines — sixty-one from init (kernel, mount, exportfs,
...
four from forktest, thirty-eight from `/rc/tests.rc` run by the REAL rc, the real sam
