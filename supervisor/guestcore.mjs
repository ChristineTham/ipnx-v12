...
async function run(mod, guardMod, asy) {
  ...
  if (WebAssembly.Module.imports(mod).some((i) => i.module === "wasi_snapshot_preview1" || i.module === "wasi_unstable")) {
    const wasi = makeWasi({ sys, sysTx, txView, m8,
      mem: () => memory, setDbg: () => {} });
    const unstable = adaptUnstable(wasi.imports);
...
// wasi_unstable (snapshot_0) over the preview1 shim: same names, two ABI diffs
// — fd_seek whence order and filestat's u32 nlink at 20 (56-byte struct).
